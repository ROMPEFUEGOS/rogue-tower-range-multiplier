...
- **No se nota nada / no aparece `[rango xN]` en el log.** Casi siempre es la configuración:
  - Comprueba que `ApplyToAllRangeCards = true`. Las cartas de rango reales (Longbow, etc.) **no** se llaman "double/triple", así que este interruptor es lo que hace que el mod actúe sobre ellas. Si tu `.cfg` lo generó una versión anterior del mod, puede haber quedado en `false` (el valor guardado gana sobre el nuevo valor por defecto).
  - **La config se lee al arrancar.** Tras editar el `.cfg`, **cierra y reabre el juego** — BepInEx aquí no recarga en caliente.
  - Verifica en `BepInEx/LogOutput.log` que el plugin carga (`Range Multiplier ... cargado`) y que cada carta de rango sale como `[carta] ... heightMult=...` seguida de `[rango xN]`.
- **El multiplicador "empieza de cero" en cada partida.** Es a propósito: se reinicia al empezar una run nueva. Rogue Tower **no guarda partidas en curso**, así que el multiplicador vale para toda esa run y cuenta desde la carta de rango que cojas (las cogidas antes de activar el mod no se arrastran).
- **El texto `+Range` del botón de construcción no cambia, pero el alcance sí.** Es por `SuppressVanillaFlatRange`: el rango real (círculo de alcance y panel de la torre) refleja el multiplicador; ese texto concreto del botón, no.
...
**How:** two Harmony patches — `TowerUpgradeCard.Upgrade` detects range cards (flat `range` ≠ 0 **or** `heightRangeBonusMultiplier` ≠ 0), accumulates a per-tower-type multiplier and refreshes towers; `Tower.SetStats` multiplies the final computed range. Multipliers reset on a new run (Rogue Tower has no mid-run save); all patches are wrapped in `try/catch` and degrade to vanilla on error.

**Troubleshooting:** if you see no effect / no `[rango xN]` line in `BepInEx/LogOutput.log`, make sure `ApplyToAllRangeCards = true` in the config (a config generated by an older build may have kept it `false`) and **restart the game** — config is only read at startup.
